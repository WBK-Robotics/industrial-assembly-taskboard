Assembly Guide
==============


The *industrial assembly challenge* taskboard is a modular benchmark developed at the wbk Institute of Production Science to evaluate and compare robotic assembly strategies.  
Each module focuses on a distinct industrial skill—from simple peg‑in‑hole insertions to assembling parts with compliant elastic deformation.

This online documentation collects every set of step‑by‑step instructions you need to **3D‑print, source hardware, and assemble** the taskboard and all of its interchangeable sub‑tasks.

How to use this guide
---------------------

1. **Print or purchase the required components** listed for each module.  
2. **Follow the illustrated assembly sequence** described on the corresponding page.  
3. **Mount the finished module** onto the taskboard base or use it stand-alone for experiments.

.. tip::

 Start with :doc:`0-Assembly_Instructions_Taskboard` to build the base board, then add tasks from the submodules as desired.


Build Instructions
------------------

.. toctree::
   :maxdepth: 1
   :caption: Module Instructions

 0-Assembly_Instructions_Taskboard
 1-Assembly-Instructions-Peg-in-Hole
 2-Assembly-Instructions-Pick-and-Place
 3-Assembly-Instructions-Gear-Assembly
 4-Assembly-Instructions-Screws-and-Nuts
 5-Assembly-Instructions-Elastic-Deformation
 List-of-Required-Components
